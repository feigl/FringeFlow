...
Help()
{
   # Display Help
    ...
    echo '   -n number of CPU processors'
    echo '   -s name of site e.g., SANEM for San Emidio or FORGE'
    echo '   -t number of track'
    echo "examples:"
    echo "    $bname -s SANEM -m S1 -1 20220331 -2 20220506 -t 42"
    echo "    $bname -s SANEM -m S1 -1 20220331 -2 20220517 -t 42 -n 4"
    echo "    $bname -s FORGE -m S1 -1 20190101 -2 20220901 -t 20"
    exit -1
  }
...
############################################################
# Get the options
while getopts ":1:2:h:m:n:s:t:" option; do
    case $option in
...
if [[ do_download -eq 1 ]]; then
    ...
    #ariaDownload.py -v --bbox '40.3480000000 40.4490000000 -119.4600000000 -119.3750000000' --output url --start 20220331 --end 20220506 --track 42 -w ./products
     ariaDownload.py -v --bbox "${BBOX}" --output url --start ${YYYYMMDD1} --end ${YYYYMMDD2} --track ${TRACK} -w ./products --output Download -nt $NPROC
 fi


# plot data
#ariaPlot.py -v -f "products/*.nc" -plotall  --figwidth=wide -nt 1
ariaPlot.py -v -f "products/*.nc" -plotbperpcoh  --figwidth=wide -nt $NPROC
...
#### Start MINTPY
mkdir -p "$RUNNAME/MINTPY_pyaps"
pushd "$RUNNAME/MINTPY_pyaps"



# make custom config file for Mintpy
echo REFLALO is $REFLALO
cat $HOME/FringeFlow/mintpy/aria_pyaps.cfg  > mintpy_aria.cfg
