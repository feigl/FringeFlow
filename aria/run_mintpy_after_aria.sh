...
Help()
{
   # Display Help
    ...
    echo "before running this script, be sure to issue"
    echo "    mamba activate mintpy"
    echo "alternatively, try this"
    echo 'run -n mintpy --live-stream bash -c "run_mintpy_after_aria.sh  -s SANEM -m S1 -1 20210331 -2 20210506 -t 42"'
    exit -1
   exit -1
  }
