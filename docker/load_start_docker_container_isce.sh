...
if [[ $(hostname) == "brady.geology.wisc.edu" ]]; then 
  ...
  popd -2
  echo Copying $HOME/FringeFlow.tgz to $TARGETDIR
  \cp -rfv $HOME/FringeFlow.tgz $TARGETDIR
fi

## copy keys here
cp -v $HOME/magic.tgz $TARGETDIR

# copy aux files
if [[ -f $HOME/aux.tgz ]]; then
   \cp -rfv $HOME/aux.tgz $TARGETDIR
else
...
if [[ -f $HOME/siteinfo.tgz ]]; then
   ...
   echo Copying $HOME/siteinfo.tgz to $TARGETDIR
   \cp -rfv $HOME/siteinfo.tgz $TARGETDIR
...
if [[ $(hostname) == "askja.ssec.wisc.edu" || $(hostname) == "maule.ssec.wisc.edu" ]]; then
  #echo "unsharing"
  podman unshare chown -R 1000:1000 $TARGETDIR
else
  echo "not unsharing"
fi
# go into container
...
if [[ $(hostname) == "askja.ssec.wisc.edu" ]] || [[ $(hostname) == "maule.ssec.wisc.edu" ]]; then
    ...
    echo sudo chown -R ${USER}:"'"domain users"'" $TARGETDIR
fi
