...
if [[ `grep $site ${HOME}/FringeFlow/siteinfo/site_dims.txt | wc -l` -gt 0 ]]; then
  if [[ `grep $site -A${coord_id} ${HOME}/FringeFlow/siteinfo/site_dims.txt | tail -1 | wc -w` -gt 0 ]]; then  
    echo $(grep $site -A${coord_id} ${HOME}/FringeFlow/siteinfo/site_dims.txt | tail -1)
  else 
