...
if [[ `grep $site ${HOME}/FringeFlow/gmtsar-aux/site_dims.txt | wc -l` -gt 0 ]]; then
  if [[ `grep $site -A${coord_id} ${HOME}/FringeFlow/gmtsar-aux/site_dims.txt | tail -1 | wc -w` -gt 0 ]]; then  
    echo $(grep $site -A${coord_id} ${HOME}/FringeFlow/gmtsar-aux/site_dims.txt | tail -1)
  else 
    echo "option not yet defined for this site."
    exit 1
  fi
else 
  echo "site ${site} not defined or site_dims.txt not found."
  exit 1
fi
